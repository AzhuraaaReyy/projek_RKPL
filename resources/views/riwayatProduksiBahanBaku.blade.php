...
<head>
    ...
    <style>
        ...
        .sidebar-logout .btn:hover {
            ...
            background: linear-gradient(135deg, #e84a5a, #d12c3d) !important;
        }
                                        ...
                                        @foreach($stokMovements as $movement)
                                            <tr>
                                                ...
                                                <td class="quantity-cell {{ $movement->movement_type == 'in' ? 'quantity-positive' : 'quantity-negative' }}">
                                                    {{ $movement->movement_type == 'in' ? '+' : '-' }}{{ number_format($movement->quantity, 2) }} {{ $movement->bahanBaku->satuan ?? 'kg' }}
                                                </td>
                                                <td><strong>{{ number_format($movement->remaining_stock, 2) }} {{ $movement->bahanBaku->satuan ?? 'kg' }}</strong></td>
                                                ...
                                                <td>
                                                    <div class="action-buttons">
                                                        ...
                                                        <button class="btn btn-action btn-edit" data-id="{{ $movement->id }}" title="Edit">
                                                            <i class="fas fa-edit"></i>
                                                        </button>
                                                        <form action="">
                                                        <button class="btn btn-action btn-delete" data-id="{{ $movement->id }}" title="Hapus">
        ...
        $(document).ready(function() {
            ...
            const table = $('#stokMovementsTable').DataTable({
                ...
                columnDefs: [
                    ...
                    { targets: [6], className: 'text-center' }, // Center align notes
                    { targets: [3, 4], className: 'text-right' } // Right align quantity columns
                ],
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                alert('Fungsi edit akan mengarahkan ke halaman edit dengan ID: ' + id);
                // window.location.href = '/riwayatBahanBaku/' + id + '/edit';
            });
